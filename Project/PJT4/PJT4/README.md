...
본 프로젝트는 관심 종목의 토론 댓글을 자동으로 수집하고 웹 애플리케이션에서 출력·삭제할 수 있도록 구축하였으며, 심화로 OpenAI API 기반 감성분석과 로고 마스크 워드클라우드 시각화를 제공한다. 기능명세서의 필수/심화 요구사항을 기준으로 설계와 구현을 완료하였다. 
...
Chrome + ChromeDriver (Selenium 크롤링용)
...
역할 분담(개발/테스트/문서) 및 구현 범위 확정

Django 앱 구조 설계 및 크롤러(Selenium) 구현

DB 모델/DAO 작성 → 저장/조회/삭제 흐름 연결

심화: OpenAI API 감성분석 및 워드클라우드 추가

소스/산출물 정리, README/문서화, Git 업로드 
...
쿼리 파라미터 ?name=삼성전자로 종목 필터 출력, 없으면 전체 출력
...
views.py의 printing 단계에서 감성분석 호출 →
...
단축키 / 또는 검색창 포커스 후 종목명 입력

최상단 결과 클릭 → 커뮤니티 탭 이동

댓글 텍스트 리스트 수집(스크롤/더보기 반복 등)

DB 모델로 변환 → bulk_create 저장
...
http://127.0.0.1:8000/crawlings/index/ 접속 → 종목명 입력

검색(크롤링) 완료 후 /crawlings/comments/?name=종목명으로 이동

상단 감성 요약/워드클라우드 확인, 댓글 리스트/삭제 기능 사용

비용 제한 조정: /crawlings/comments/?name=종목명&sa=20 (SA 20개만 분석)
...
마스크 이미지가 컬러일 때: 코드에서 이진화로 형태를 강제
...
구현 소스 코드: Django 프로젝트 및 crawlings 앱 전 소스

실행 결과 캡처: DB/화면(입력, 목록, 삭제, 요약/워드클라우드)

README.md(본 문서): 구현 설명/학습 내용/개선 포인트 포함
→ Git에 업로드, 프로젝트명은 04_pjt로 지정하여 제출. 
