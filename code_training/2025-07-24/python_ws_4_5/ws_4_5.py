...
def is_validation(dic):
    ...
    for key in dic:
        if key == 'blood_group':
            if dic['blood_group'] not in blood_types:
                wrong_list.append('blood_group')
        elif key == 'company':
            if dic['company'] in black_list:
                return 'blocked' 
        elif key == 'mail':
            if '@' not in dic['mail']:
                wrong_list.append('mail')
        elif key == 'name':
            if (2 > len(dic['name'])) or (len(dic['name']) > 30):
                wrong_list.append('name')
        else:
            if 1 > len(dic['website']):
                wrong_list.append('name')
    if wrong_list:
...
def create_user(lst):
    ...
    for dic in lst:
        ...
        if result == True:
            ...
            wrong_user += 1
            continue
    return user_list, wrong_user

result_list, result_wrong = create_user(user_data)
if result_wrong != 0:
    print(f'잘못된 데이터로 구성된 유저의 수는 {result_wrong} 입니다.')
print(result_list)